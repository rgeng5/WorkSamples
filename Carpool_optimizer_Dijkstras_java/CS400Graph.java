...
// Notes to Grader: <This is my implementation of Dijkstra's algorithm with portions provided by course team.>
...
public class CS400Graph<T> implements GraphADT<T> {
	...
	Vertex v0 = new Vertex(start);
   	Hashtable<T, Vertex> verticesUsed;
	verticesUsed = new Hashtable<>();
	verticesUsed.put(start, v0);
	
	Path p = new Path(v0);
	
	while (getVertexCount() >= verticesUsed.size()) {	
		PriorityQueue<Path> queue = new PriorityQueue<>();
		int count_t = 0;
		for (T t : p.dataSequence) {
			for(Edge e : vertices.get(t).edgesLeaving) {
				if (! verticesUsed.containsKey(e.target.data)) {
					Path p2 = new Path(p, e);
					queue.add(p2);
				}
			}
		}

		Path shortPath = queue.peek();
		Vertex VertexToAdd = shortPath.end;
		verticesUsed.put(shortPath.end.data, shortPath.end);

		if (shortPath.end.data.equals(end)) {
			return shortPath;
		}

		p = shortPath;
	}
	throw new NoSuchElementException("No path found between these vertices");
    }
